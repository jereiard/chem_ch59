- [59장](#59%ec%9e%a5)
  ...
  - [Human pregnancy](#human-pregnancy)
    ...
    - [Functional Developmenf of the Fetus](#functional-developmenf-of-the-fetus)
  ...
  - [Prenatal Screening for Fetal Defects](#prenatal-screening-for-fetal-defects)
    - [```Terminology and Method of Risk Calculation in Prenatal Screening```](#terminology-and-method-of-risk-calculation-in-prenatal-screening)
      ...
      - [```Calculating Individualized Patient-Specific Risks Using Multiple Biochemocal measurements```](#calculating-individualized-patient-specific-risks-using-multiple-biochemocal-measurements)
...
임상적으로 양수의 병적 감소 및 증가가 자주 발생합니다. 자궁 내 성장 지연 및 태아 요로의 이상(예를 들어, 양측 신장 생성 또는 요도 폐쇄)은 비정상적으로 낮은 양수량인 양수과소증(oligohydroamios)와 관련이 있습니다. 반대의 경우를 양수과다증(hydramnios)이라고도합니다. 양수과다증은 (1) 모성 당뇨병, (2) 심한 Rh isoimmune 병, (3) 태아 식도 폐쇄증, (4) 다태아 임신, (5) 뇌전 증 및 (6) spina bifida과 관련되어 있습니다.
...
### Functional Developmenf of the Fetus
...
#### ```Calculating Individualized Patient-Specific Risks Using Multiple Biochemocal measurements```
...
Risks are calculated using the adjusted MoM values along with the woman's maternal age at expected delivery by employing an algorithm.  
...
특정 조건에 대해 개빌화된 위험도(환자별 위험) = 특정 조건에 대한 사전-정의된 위험 × Likelihood ratio(가능성비율) 
...
생략. 제 1삼분기에서 시행하는 combined test의 한계들에 대해서 설명함 (다음 섹션에서 소개할 검사방법이 더 낫다는 것을 설명하기 위한 밑밥).
